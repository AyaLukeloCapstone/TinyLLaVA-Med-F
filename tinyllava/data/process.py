...
#             # print("Type of sentence['value']:", type(sentence['value']))
#             # print("Content of sentence['value']:", sentence['value'])
#             # Ensure sentence['value'] is a string
#             sentence_value = str(sentence['value'])
...
def preprocess_multimodal(sources: Sequence[str], data_args: DataArguments) -> Dict:
    ...
    for source in sources:
        for sentence in source:
            ...
            # Replace DEFAULT_IMAGE_TOKEN if present
            if DEFAULT_IMAGE_TOKEN in sentence_value:
                sentence_value = sentence_value.replace(DEFAULT_IMAGE_TOKEN, '').strip()
                sentence_value = DEFAULT_IMAGE_TOKEN + '\n' + sentence_value
                sentence_value = sentence_value.strip()
                if "mmtag" in conversation_lib.default_conversation.version:
                    sentence_value = sentence_value.replace(DEFAULT_IMAGE_TOKEN,
                                                            '<Image>' + DEFAULT_IMAGE_TOKEN + '</Image>')

            # Handle start and end image tokens
            replace_token = DEFAULT_IMAGE_TOKEN
            if data_args.mm_use_im_start_end:
                replace_token = DEFAULT_IM_START_TOKEN + replace_token + DEFAULT_IM_END_TOKEN
            sentence['value'] = sentence_value.replace(DEFAULT_IMAGE_TOKEN, replace_token)
