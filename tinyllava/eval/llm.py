...
class GPT(LLM):
  ...
  def __init__(self, model_id):
    ...
    self.top_k = 1
    # self.encoding = tiktoken.encoding_for_model("-".join(model_id.split("-", 2)[:2]).replace('5', '.5')) # LINE CAUSING errors during the GPT evaluation 
    self.encoding = tiktoken.encoding_for_model("gpt-4o") # this line works - but i am not sure it is correct 
    # self.encoding = tiktoken.get_encoding("AutoTokenizer")
